...
prop_info = pd.read_csv('dataset/Prop_info.csv')
raw_data = pd.read_csv('dataset/Forecast_property.csv')
...
if st.session_state.page_name == 'forecast':
	...
	if st.sidebar.button('**Home Page**'):
		...
		st.session_state.page_name = ''
		st.experimental_rerun()
	raw = pd.read_csv('dataset/Prop_info.csv').drop(columns=['areaUnit'])
